...
class HopNet(val size: Int) extends Simulable {
  ...
  def setMaxG(maxg: Double) = synapses foreach { arr ⇒ arr foreach { syn ⇒ if (syn != null) syn.maxg = maxg } }
  def setMinG(ming: Double) = synapses foreach { arr ⇒ arr foreach { syn ⇒ if (syn != null) syn.ming = ming } }
  ...
  def createLDC(): LargeDataCollector = {
    if (ldc.isEmpty) {
      ldc = Some(new LargeDataCollector(getCollectFunc) { metadata = getCollectMeta.toArray })
