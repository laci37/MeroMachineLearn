...
class Window extends JFrame {
  ...
  var net = new ContextHopNet(12 * 8 * 2 ,1024, 0d) with Reward
  ...
  def loadInputs(img: BufferedImage, code: Int) = {
    ...
    for (i <- (0 to 7)) for (j <- (0 to 11)) {
      ...
      net.inputs((i * 12 + j) * 2) set in
      net.inputs((i * 12 + j) * 2 + 1) set 1 - in
    }
  }
  ...
  def drawOutput(): BufferedImage = {
    ...
    for (i <- (0 to 7)) for (j <- (0 to 11))
      img.setRGB(i, j, net.outputs((i * 12 + j) * 2).toInt * 0xFFFFFFFF)
  ...
  override def paint(g: Graphics) = {
    ...
    val trf=new java.awt.geom.AffineTransform()
    trf.scale(10,10)
